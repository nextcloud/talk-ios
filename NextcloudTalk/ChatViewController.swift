    ...
    func startCall(withVideo video: Bool, silently: Bool, button: BarButtonItemWithActivity) {
        if self.room.recordingConsent {
            ...
            alert.addAction(.init(title: NSLocalizedString("Give consent and join call", comment: "Give consent to the recording of the call and join that call"), style: .default) { _ in
                CallKitManager.sharedInstance().startCall(self.room.token, withVideoEnabled: video, andDisplayName: self.room.displayName, asInitiator: true, silently: silently, recordingConsent: true, withAccountId: self.room.accountId)
            })
            ...
            CallKitManager.sharedInstance().startCall(self.room.token, withVideoEnabled: video, andDisplayName: self.room.displayName, asInitiator: true, silently: silently, recordingConsent: false, withAccountId: self.room.accountId)
        }
    }
