...
- (void)setupForMessage:(NCChatMessage *)message withLastCommonReadMessage:(NSInteger)lastCommonRead
{
    ...
    // if media file, we don't need to show the file name since it's usually autogenerated
    self.bodyTextView.text = isMediaFile ? @"" : self.bodyTextView.text;  // or [self.bodyTextView removeFromSuperView] or self.bodyTextView.hidden = YES
    self.previewImageView.layer.cornerRadius = isMediaFile ? 8.0 : 4.0;
    // TODO: if isMediaFile, remove the "last row" containing statusView and bodyTextView, and bring the statusView up to be flush with the bottom of the image
