...
class OpenConversationsTableViewController: UITableViewController, UISearchResultsUpdating {
    ...
    func searchForListableRooms() {
        NCAPIController.sharedInstance().getListableRooms(forAccount: .activeAccount, withSerachTerm: "") { listableRooms, _ in
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.avatarView.setGroupAvatar()
        // Try to get room avatar even though at the moment (Talk 17) it is not exposed
        cell.avatarView.setAvatar(for: openConversation)
        // Set description
