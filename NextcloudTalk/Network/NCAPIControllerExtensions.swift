...
@objc extension NCAPIController {
    ...
    public func getBots(forRoom token: String, forAccount account: TalkAccount) async throws -> [Bot] {
        ...
        let apiVersion = self.botsAPIVersion(forAccount: account)
        let urlString = self.getRequestURL(forEndpoint: "bot/\(encodedToken)", withAPIVersion: apiVersion, forAccount: account)
    ...
    public func enableBot(withId botId: Int, forRoom token: String, forAccount account: TalkAccount) async throws -> Bot? {
        ...
        let apiVersion = self.botsAPIVersion(forAccount: account)
        let urlString = self.getRequestURL(forEndpoint: "bot/\(encodedToken)/\(botId)", withAPIVersion: apiVersion, forAccount: account)
